## 6 - Install SubCA1 (Lab Issuing CA 1) - PART 2 (YubiHSM 2 Edition)
...
### NOTE: certutil -installcert will automatically find the matching private key on the YubiHSM.

# --- Logging: ensure folder and start transcript ---
$LogDir = "C:\Scripts"
if (!(Test-Path $LogDir)) { New-Item -Path $LogDir -ItemType Directory -Force | Out-Null }
Start-Transcript -Path (Join-Path $LogDir "PKILab-6a-Subca1-Install-Part2.log") -Append -ErrorAction SilentlyContinue
...
# YUBIHSM NOTE: certutil will automatically match this cert to the private key stored on the YubiHSM
...
# Local path publish only - NOT embedded in issued certs (flag 1 = publish only)
...
$aiaOutput | Where-Object { $_ -match '^\s+\d+:\s+\d+:' } | ForEach-Object {
    if ($_ -match '^\s+\d+:\s+(\d+):(.+)$') {
        ...
        } elseif ($url -match 'ocsp' -and $url -notmatch [regex]::Escape($expectedOCSP)) {
